...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const get = await axios.get("/api/penduduk");
      setPost(get.data);
    };
    fetchData();
  }, [isUploaded]);
  ...
  const handlerDelete = async (e) => {
    ...
    try {
      const del = await axios.delete(`/api/penduduk/${key}`);
      if (del.data.success) {
        setIsUploaded(true);
      }
    } catch (err) {}
  ...
  const handlerSubmit = async (e) => {
    ...
    try {
      const post = await axios.post("/api/penduduk", data);
      ...
      <CardBody>
        ...
        <Card className="h-full w-full mt-4 overflow-scroll">
          <table className="w-full min-w-max table-auto text-left">
            ...
            <tbody>
              {post &&
                post.data.map(({ url, id, content }, index) => {
                  return (
                    <tr key={id}>
                      <td className={`p-4`}>
