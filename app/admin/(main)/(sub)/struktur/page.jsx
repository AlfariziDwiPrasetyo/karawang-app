...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const get = await axios.get("/api/struktur");
      setBanner(get.data);
    };
    fetchData();
    // setIsMount(false);
  ...
  const handlerDelete = async (e) => {
    ...
    try {
      const del = await axios.delete(`/api/struktur/${key}`);
      if (del.data.success) {
        setIsUploaded(true);
      }
    } catch (err) {}
  ...
  const handlerSubmit = async (e) => {
    ...
    try {
      const post = await axios.post("/api/struktur", data);
